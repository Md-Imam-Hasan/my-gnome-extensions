...
export class ClipboardService {
    ...
    start() {
        ...
        this._ownerChangedId = this._selection.connect('owner-changed', (_, selectionType) => {
            if (selectionType === Meta.SelectionType.SELECTION_CLIPBOARD)
                this._scheduleRead();
        });

        log(`${LOG} monitoring clipboard (Meta.Selection owner-changed + St.Clipboard)`);
    }
        ...
        try {
            this._stClipboard.get_text(St.ClipboardType.CLIPBOARD, (cb, text) => {
                try {
                    ...
                    if (!Format.isWithinByteLimit(normalized))
                        return;
