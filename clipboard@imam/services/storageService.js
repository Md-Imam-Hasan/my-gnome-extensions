...
export class StorageService {
    constructor(store, security) {
        ...
        this._dirPath = GLib.build_filenamev([home, '.local', 'share', C.STORAGE_FOLDER_NAME]);
        this._historyPath = GLib.build_filenamev([this._dirPath, C.HISTORY_FILENAME]);
    ...
    _readConfigFile() {
        try {
            ...
            if (typeof o.persist === 'boolean')
                this._config.persist = o.persist;
            if (typeof o.encrypt === 'boolean')
                this._config.encrypt = o.encrypt;
        } catch {
            /* keep defaults */
        }
    }
    ...
    _loadHistorySync() {
        if (!this._historyFile || !this._historyFile.query_exists(null))
            return;
    ...
    _decryptAndApplyAsync(root) {
        ...
        this._security.decryptPayload({
            ...
            const wrapped = { version: 1, encrypted: false, entries: inner.entries };
    ...
    scheduleSave() {
        ...
        this._saveTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, C.SAVE_DEBOUNCE_MS, () => {
            this._saveTimeoutId = 0;
            this._saveNow().catch(() => {
                log(`${LOG} save failed`);
            });
     ...
     */
    async _saveNow() {
        if (!this._config.persist || !this._historyFile)
            return;
        ...
        try {
            if (this._config.encrypt && this._security.encryptionAvailable) {
                const pass = this._security.getPassphrase() ||
                    (C.DEV_FALLBACK_PASSPHRASE.length > 0 ? C.DEV_FALLBACK_PASSPHRASE : null);
                if (!pass) {
                ...
                this._writeHistoryBytes(new TextEncoder().encode(JSON.stringify(outer)));
            } else {
                const plain = {
                    version: 1,
                    encrypted: false,
                ...
                this._writeHistoryBytes(new TextEncoder().encode(JSON.stringify(plain)));
            }
        } catch {
