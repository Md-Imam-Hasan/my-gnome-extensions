...
export default class ClipboardManagerExtension extends Extension {
  ...
  enable() {
    ...
    this._lockUnsub = this._storage.subscribeLock((locked) => {
      if (this._indicator) this._indicator.setLocked(locked);
    });

    import("resource:///org/gnome/shell/misc/config.js")
      .then((Config) => {
        log(
          `[clipboard-manager@muhammad-imam-hasan.com] extension enabled — GNOME Shell ${Config.PACKAGE_VERSION ?? "?"}`,
        );
      })
      .catch(() => {
        log(
          "[clipboard-manager@muhammad-imam-hasan.com] extension enabled (shell version unavailable)",
        );
      });
  }
