...
export class SecurityService {
  ...
  _detectCrypto() {
    ...
    log(
      `${LOG} crypto.subtle: ${this.encryptionAvailable ? "yes" : "no"} (journalctl to verify)`,
    );
  ...
  async encryptPayload(plainText) {
    if (!this.encryptionAvailable) throw new Error("encryption unavailable");
  ...
  setLocked(failed, reason) {
    ...
    if (failed) log(`${LOG} storage locked (${reason ?? "decrypt failed"})`);
  }
