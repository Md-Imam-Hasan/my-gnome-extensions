...
export class ClipboardDropdown {
  ...
  _build() {
    ...
    this._searchEntry.connect("notify::text", () => {
      if (this._searchDebounceId) GLib.source_remove(this._searchDebounceId);
    ...
    for (const key of ["all", "text", "url", "code", "other"]) {
      const it = new M.PopupMenuItem(
        CATEGORY_LABEL[/** @type {keyof typeof CATEGORY_LABEL} */ (key)],
      );
      it.connect("activate", () => {
        this._category = /** @type {CategoryFilter} */ (key);
        this._catSub?.label.set_text(
          `Category: ${CATEGORY_LABEL[/** @type {keyof typeof CATEGORY_LABEL} */ (key)]}`,
        );
        this._renderFiltered();
      });
    ...
    const hint = new M.PopupMenuItem("Star = pin · Click row = copy", {
      reactive: false,
      can_focus: false,
    });
    ...
    this._menuOpenStateId = this._menu.connect(
      "open-state-changed",
      (_menu, open) => {
        if (open) this._updateHistoryScrollMaxHeight();
      },
    );
    ...
    this._encryptItem.connect("activate", () => {
      if (!this._security.encryptionAvailable) return;
      const c = this._storage.getConfig();
      this._storage.setConfig(c.persist, !c.encrypt);
      this._syncToggleLabels();
    });
    ...
    this._encPassEntry.connect("notify::text", () => {
      ...
      if (t.length > 0) this._security.setPassphrase(t);
    ...
    this._clearItem.connect("activate", () => {
      try {
        ...
        console.error(
          "[clipboard-manager@muhammad-imam-hasan.com] clear history",
          e,
        );
      }
    });
  ...
  _updateHistoryScrollMaxHeight() {
    if (!this._historyScrollView) return;
    let screenH = 1080;
    try {
      const mon = Main.layoutManager.currentMonitor;
      if (mon) screenH = mon.height;
      else {
  ...
  _syncLockVisibility() {
    ...
    if (this._searchEntry?.get_parent()) this._searchEntry.reactive = !locked;
  }
  ...
  _renderFiltered() {
    ...
    try {
      ...
      if (this._storage.isLocked()) {
        const msg = new PopupMenu.PopupMenuItem(
          "History locked — enter passphrase and Unlock",
          {
            reactive: false,
            can_focus: false,
          },
        ...
        if (this._clearItem) this._clearItem.setSensitive(false);
        return;
      }
      ...
      if (filtered.length === 0) {
        ...
        for (const entry of filtered) {
          ...
          if (markup) row.label.clutter_text.set_markup(markup);
          else row.label.set_text(previewRaw);
          ...
          pinBtn.connect("clicked", () => {
            try {
              ...
              console.error(
                "[clipboard-manager@muhammad-imam-hasan.com] pin toggle",
                e,
              );
            }
          });
          ...
          row.connect("activate", (menuItem, event) => {
            ...
            if (this._eventTargetsPinButton(event, pinBtn)) return;
            try {
              ...
              console.error(
                "[clipboard-manager@muhammad-imam-hasan.com] re-copy",
                e,
              );
            }
          });
      ...
      if (this._clearItem) this._clearItem.setSensitive(all.length > 0);
    } catch (e) {
      console.error(
        "[clipboard-manager@muhammad-imam-hasan.com] render history",
        e,
      );
    }
  }
